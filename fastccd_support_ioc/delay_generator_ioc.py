...
class DelayGenerator(PVGroup):
    ...
    trigger_on_off = pvproperty_with_rbv(dtype=float, doc="TriggerOnOFF")
    delay_time = pvproperty_with_rbv(dtype=float, doc="DelayTime")
    shutter_time =  pvproperty_with_rbv(dtype=float, doc="ShutterTime")

    #additional PV only used for setup
    setup = pvproperty_with_rbv(dtype=float, doc="Setup")


    @setup.setpoint.putter
    async def setup(obj, instance, init):
        if init == 1:
